...
def Select_by_sid(sid, close, open_, high , low, vol, rev, com_rev, d_yield, pb, benchmark):
    ...
    # Serial Correlation (Optimized with Numba - much faster!)
    window_autocorr = 50
    
    # Import the fast Numba function from features.py
    from features import rolling_autocorr_numba
    
    # Calculate base autocorrelation using fast Numba function
    data['autocorr_1'] = rolling_autocorr_numba(data['log_ret'].values, lookback=window_autocorr)
    
    # Create lagged versions
    data['autocorr_2'] = data['autocorr_1'].shift(1)
    data['autocorr_3'] = data['autocorr_1'].shift(2)
    data['autocorr_4'] = data['autocorr_1'].shift(3)
    data['autocorr_5'] = data['autocorr_1'].shift(4)
