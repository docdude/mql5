...
def drop_labels(triple_barrier_events, min_pct=0.05):
    ...
    while True:
        ...
        logging.info(f"Dropped label: {df0.idxmin()} - {df0.min():.4%}")
...
def triple_barrier_labels(
    ...
    if verbose:
        ...
        bin_counts = events.bin.value_counts(normalize=True)
        print(f"\nLabel distribution:")
        for label in sorted(bin_counts.index):
            print(f"  {label}: {bin_counts[label]:.2%}")
        
        # Print accuracy if label 1 exists
        if 1 in bin_counts.index:
            print(f"\nAccuracy (positive labels): {bin_counts[1]:.2%}")
