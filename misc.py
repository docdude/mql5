...
def log_column_changes(func):
    ...
    def wrapper(df: pd.DataFrame, *args, **kwargs) -> pd.DataFrame:
        ...
        if old_cols != new_cols:
            logging.info("Column names changed:")
            for old, new in zip(old_cols, new_cols):
                if old != new:
                    logging.info(f"  '{old}' -> '{new}'")
        else:
            logging.info("No column name changes detected.")
        return result
...
def log_performance(func):
    ...
    :type func: Callable
    :return: A wrapped function that logs performance metrics to the configured logging.
    :rtype: Callable
